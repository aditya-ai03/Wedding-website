...
const EventItem = ({ title, date, time, description, icon, isLast = false, index }: EventItemProps) => {
  // Logic Reversed: 
  // index 0, 2, 4 (Even) -> Title | Line | Details + Icon
  // index 1, 3, 5 (Odd)  -> Icon + Title | Line | Details
  ...
  return (
    <div className={`relative ${!isLast ? 'border-b border-[#C5A059]/30' : ''} py-10 md:py-16`}>
      <div className="flex flex-row items-stretch gap-4 md:gap-10">
        ...
        {/* 1. LEFT SIDE: Swaps between (Icon + Title) and (Just Title) */}
        <div className={`w-1/3 md:w-[35%] flex items-center gap-3 md:gap-8 ${isLayoutReversed ? 'flex-row-reverse' : 'flex-row'} justify-end`}>
          {/* Title Section */}
          ...
          {/* Icon Section - Shows on Left only if layout is reversed (Odd indices) */}
          {isLayoutReversed && icon && (
        ...
        {/* 3. RIGHT SIDE: Swaps between (Details) and (Details + Icon) */}
        <div className="flex-grow flex items-center gap-4 md:gap-8 justify-between">
          <div className="flex-grow pt-1 text-left">
            <p className="font-lora text-[10px] md:text-xs font-bold text-[#5C2336] uppercase tracking-[0.2em] mb-1">
              {date}
            </p>
            <p className="font-lora text-[10px] md:text-xs font-bold text-[#5C2336] uppercase tracking-widest mb-4">
              {time}
            </p>
          ...
          {/* Icon Section - Shows on Right only if layout is NOT reversed (Even indices) */}
          {!isLayoutReversed && icon && (
            <div className="flex-shrink-0">
              <div className="w-14 h-14 md:w-28 md:h-28">
                <img src={icon} alt="" className="w-full h-full object-contain opacity-90" />
...
const EventsTimeline = () => {
  const events = [
    ...
    },
    {
      ...
      date: "MONDAY, OCTOBER 26, 2026",
      time: "6 PM ONWARDS",
      description: "Our Sangeet and mehndi will start in the early evening and flow late into the night with henna, music, and dancing. This is where the celebrations truly begin.",
      icon: sangeetIcon,
    },
    {
      ...
      time: "8 AM",
      description: "A Ganesh Pooja and blessing ceremony to begin the wedding day with intention, gratitude, and grace. Your prayers and good wishes mean the world to us.",
      icon: blessings,
    },
    {
      title: "Haldi Hues",
      date: "TUESDAY, OCTOBER 27, 2026",
      time: "11 AM ONWARDS",
      description: "A joyful Haldi ceremony filled with colour, laughter, and tradition. Wear something you won't be too attached to!",
      icon: haldiIcon,
    },
    {
      title: "The Wedding",
      ...
      description: (
        <>
          <p className="font-bold text-[#5C2336]">Jaan Entrance | 4:00 PM</p>
          <p className="mb-2">The groom’s arrival, filled with music and energy.</p>
          <p className="font-bold text-[#5C2336]">Ceremony | 5:30 PM</p>
    ...
    },
    {
      ...
      time: "11 AM ONWARDS",
      description: "A high-energy daytime celebration with music, colour, and just enough chaos to set the tone for the final night. Pace yourselves!",
      icon: poolpartyIcon,
    },
    {
      ...
      date: "WEDNESDAY, OCTOBER 28, 2026",
      time: "7 PM ONWARDS",
      description: "Dinner, music, dancing, and one last night together to celebrate our journey and new beginning.",
      icon: finaleIcon,
    },
  ];
